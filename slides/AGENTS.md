...
- **10-12 matplotlib charts** (generated from Python scripts, flat in the lecture dir)
- **2 cartoons** (opening and closing, XKCD style)
- **AGENTS.md** documentation for the lecture

**Current status:**
- L01, L02: Fully implemented
- L03-L07: Planned (metadata defined in `../config.yaml`)
...
**Figures:** 10 charts + 2 cartoons (all flat in lecture dir)
...
**Status:** Metadata defined in `../config.yaml`
...
### 1. Create folder
```bash
mkdir slides/L0N_[Slug]
cd slides/L0N_[Slug]
```

### 2. Create slide variants
...
### 3. Create chart subdirectories (flat in lecture dir)

Each chart subdirectory:
```
L0N_[Slug]/
├── 01_[descriptor]/
│   ├── chart.py
...
PDFs and assets from slides are published to the website:

- **docs/slides/pdf/**: All 6 variants per lecture (e.g., `L01_full.pdf`, `L01_mini5.pdf`)
- **docs/slides/images/L[N]/**: PNG exports of all figures (PDF→PNG conversion)
- **docs/galleries/images/L[N]/**: Slide gallery previews by variant (mini5, mini10, core, extended, full)
...
- **Siblings:**
  ...
  - `../config.yaml` — Course metadata (lecture titles, topics, prerequisites)
