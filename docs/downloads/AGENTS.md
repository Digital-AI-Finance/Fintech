...
The **downloads/** directory contains compiled PDF files in 6 variants per lecture, ready for download and offline viewing. These PDFs are derived from source materials in the lectures directory and mirror the slide variants available in the gallery (mini5, mini10, core, deepdive, overview, full). All PDFs are static files—no processing, no server logic required.
...
The downloads directory PDFs correspond to the gallery image variants:
...
1. **Export slide variants** from source slide deck (PowerPoint, Keynote, etc.) in PDF format
2. **Name files** according to convention (L01_mini5.pdf, L01_mini10.pdf, etc.)
3. **Move to downloads/** directory
4. **Test downloads** (verify file integrity, test open in PDF readers)

**Tools** (if re-compiling):
- LibreOffice Impress: Export → PDF (select page ranges)
- Microsoft PowerPoint: Print → Print to PDF (select page ranges)
...
2. **Re-export PDF** from source (page range: e.g., slides 1–6 for mini5)
3. **Replace file** in downloads/ directory
...
```html
<a href="downloads/L01_mini5.pdf" download>Download mini5 PDF</a>
<a href="downloads/L01_full.pdf" download>Download Full PDF</a>
```
...
```
/downloads/L01_full.pdf
/downloads/L02_overview.pdf
etc.
```
...
1. **Compile 6 PDF variants** from source materials (or export from slide deck)
...
1. **Update source slide deck** (if content changed)
2. **Export PDF** from presentation software (select correct page range)
3. **Save** with exact filename (L01_mini5.pdf, etc.)
4. **Replace file** in downloads/ directory
...
```html
<!-- Old -->
<a href="downloads/L01_mini.pdf" download>Download Mini</a>

<!-- New -->
<a href="downloads/L01_mini5.pdf" download>Download mini5 (6 slides)</a>
```

Ensure href matches actual filename in downloads/ directory.
...
- **AVOID**: Hosting PDFs on third-party services—keep them in downloads/ directory.
...
- **Charts**: `../charts/AGENTS.md` — Chart images (may be embedded in PDFs)
- **Source Materials**: `D:/Joerg/Research/slides/Fintech/lectures/` — Original slide decks
